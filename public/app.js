...
function authPanel() {
  return `
    <section class="panel">
      <h2>先进入你们的工作区</h2>
      <p class="copy">先建立或打开本机用户，之后两个人会看到同一份婚礼计划。</p>
      <form class="form-grid" data-form="register">
        ...
        <button type="submit">建立 / 打开用户</button>
      </form>
      <form class="form-grid" data-form="login" style="margin-top: 12px;">
        <label>打开已有用户<input name="email" type="email" placeholder="partner@example.com"></label>
        <button class="secondary" type="submit">用 Email 打开</button>
      </form>
    </section>
  `;
}
  ...
  return `
    <section class="side-rail">
      ...
      <nav class="side-nav" aria-label="主功能">
        ...
        <a href="#tasks">任务管理</a>
        <a href="#timeline">日程时间线</a>
        <a href="#budget">预算管理</a>
        <a href="#guests">宾客管理</a>
      </nav>
...
function mainApp() {
  if (!state.user) {
    return `<main class="layout">${authPanel()}${invitePanel() || `<section class="hero-panel"><h2>开始 O&S 婚礼追踪。</h2><p class="copy">先建立用户，再用 3 个资料生成你们的华人婚礼 dashboard。</p></section>`}</main>`;
  }
  if (state.inviteToken && state.invite && !state.data) {
    return `<main class="layout"><div class="section-stack">${authPanel()}${invitePanel()}</div><section class="hero-panel"><h2>加入后一起更新。</h2><p class="copy">接受邀请后，你们会拥有同一份婚礼追踪资料。</p></section></main>`;
  }
  if (!state.data) {
  ...
  return `
    <main class="dashboard-layout" id="dashboard">
      ...
      <section class="dashboard-main">
        ...
        <div class="data-grid update-grid">
          ${ceremonyPanel()}
          ${taskPanel()}
          ${budgetPanel()}
          ${guestPanel()}
